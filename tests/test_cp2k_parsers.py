...
class TestSelectBasisByAccuracy:
    ...
    def test_single_entry_below_target(self):
    ...
    def test_fallback_when_none_meets_target(self):
        entries = [
            self.make_entry("coarse", 3e-6),
            self.make_entry("finer", 8e-7),
        ]
        result = _select_basis_by_accuracy(entries, 1e-5)
        assert result == "finer"
...
""")
        ...
        assert result == "RI_basis_finer_error_3e-07"
